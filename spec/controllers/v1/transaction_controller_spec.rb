...
RSpec.describe Api::V1::TransactionsController, type: :request do
  ...
  describe '#index' do
    ...
    context 'when login as admin' do
      ...
      context 'and query string has amount' do
        let!(:send_request) { get "#{endpoint}?#{query_string}&amount=5:#{@transactions[0].amount}", headers: auth_header(admin_token) }
        let!(:filtered_transactions) { @transactions.select { |t| t.amount <= @transactions[0].amount && t.amount >= 5 } }

        it { expect(response_status).to eq 200} 
        it { expect(response_data.size).to eq filtered_transactions.size }
      end

      context 'and query string has amount and before date' do
        let!(:send_request) { get "#{endpoint}?#{query_string}&amount=:#{@transactions[0].amount}&before=#{@transactions[0].created_at + 1.day}", headers: auth_header(admin_token) }
        let!(:filtered_transactions) { @transactions.select { |t| t.amount <= @transactions[0].amount && t.created_at <= (@transactions[0].created_at + 1.day) } }
  ...
  describe '#create' do
    ...
    context 'when login as normal user' do
      context 'and purchase data is valid' do
        let(:purchase_params) { products.map { |p| { product_id: p.id, quantity: p.quantity - 1 } } }
        let(:params) { { transaction: { purchased_products_attributes: purchase_params, genre: 'purchase' } } }
        let!(:send_request) { post endpoint, params: params, headers: auth_header(token) }
      ...
      context 'and products are out of stock' do
        let(:invalid_params) { products.map { |p| { product_id: p.id, quantity: p.quantity + 10 } } }
        let(:params) { { transaction: { purchased_products_attributes: invalid_params, genre: 'purchase' } } }
        let!(:send_request) { post endpoint, params: params, headers: auth_header(token) }
      ...
      context 'and transfer data is valid' do
        let(:transfer_params) { [{ amount: 100, receiver_id: receiver.account.id }] }
        let(:params) { { transaction: { genre: 'transfer', transfer_details_attributes: transfer_params } } }
        let!(:send_request) { post endpoint, params: params, headers: auth_header(giver_token) }
      ...
      context 'and transfer data is invalid' do
        ...
        let(:params) { { transaction: { genre: 'transfer', transfer_details_attributes: invalid_params } } }
